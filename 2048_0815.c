...
void loadHighestScore() {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        ...
        snprintf(file_path, sizeof(file_path), "%s/highest_score.txt", cwd);
...
int main() {
    ...
    // Reset the game map and score for a new game
    for (int i = 0; i < SIZE; i++) {
        for (int j = 0; j < SIZE; j++) {
            map[i][j] = 0;
        }
    ...
    while (true) {
        ...
        if (judgeover_val == 0) {
            ...
            printf(ANSI_COLOR_UNDERLINE "Would you like to play again? Press any key to continue or N to exit. \n" ANSI_COLOR_RESET);
            char input[10];
            scanf("%s", input);
            if (strcmp(input, "n") == 0 || strcmp(input, "N") == 0) {
                printf("Exiting the program...\n");
                break; // Exit the inner loop and return to the start screen
            }
